...
async def list_jobs() -> dict:
    ...
    """
    jobs = await progress_tracker.get_all()
    return {
        "jobs": [
            (await progress_tracker.get_job_stats(j.job_id))
            or {
                "job_id": j.job_id,
...
async def get_job(job_id: str) -> dict:
    ...
    """
    stats = await progress_tracker.get_job_stats(job_id)
    if not stats:
...
async def delete_job(
    ...
    job_info = await progress_tracker.get(job_id)
