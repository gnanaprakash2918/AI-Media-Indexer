...
async def get_video_overlays(
    ...
    start_time: Annotated[float | None, Query(description="Start time in seconds")] = None,
    end_time: Annotated[float | None, Query(description="End time in seconds")] = None,
):
    ...
    try:
        ...
        frames = pipeline.db.get_frames_by_video(
            video_path=video_id,
            start_time=start_time,
            end_time=end_time,
        ) if hasattr(pipeline.db, 'get_frames_by_video') else []
        ...
        for frame in frames:
            ...
            for i, bbox in enumerate(face_boxes):
                overlays["faces"].append({
                    "timestamp": ts,
                    "bbox": bbox,
                    "label": frame.get("face_names", [])[i] if i < len(frame.get("face_names", [])) else None,
                    "cluster_id": frame.get("face_cluster_ids", [])[i] if i < len(frame.get("face_cluster_ids", [])) else None,
                    "color": "#22C55E",
                })
        ...
        loudness_events = pipeline.db.get_loudness_events(
            video_path=video_id,
            start_time=start_time,
            end_time=end_time,
        ) if hasattr(pipeline.db, 'get_loudness_events') else []
        ...
        logger.info(f"[Overlays] {video_id}: {len(overlays['faces'])} faces, {len(overlays['text_regions'])} text, {len(overlays['objects'])} objects")
        return overlays
...
async def get_frame_overlays(
    ...
    try:
        frame = pipeline.db.get_frame_by_id(frame_id) if hasattr(pipeline.db, 'get_frame_by_id') else None
        if not frame:
