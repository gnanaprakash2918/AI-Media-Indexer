...
class TagObjectRequest(BaseModel):
    ...
    point_labels: Optional[List[int]] = (
        None  # 1 for foreground, 0 for background
    )
    start_frame: int = 0
...
async def tag_object(
    request: TagObjectRequest, background_tasks: BackgroundTasks
):
    ...
    background_tasks.add_task(_run_tracking_task, request)
    ...
    try:
        ...
        if req.concept_label:
            ...
            segments = await tracker.track_concept(
                req.video_path, req.concept_label, start_time=req.start_time
            )
        elif req.points:
        ...
        for seg in segments:
            ...
            try:
                visual_vector = await tracker.extract_visual_embedding(
                    req.video_path, mask, frame_idx
                )
            except Exception as e:
                log.warning(
                    f"[Tracking] Embedding extraction failed for frame {frame_idx}: {e}"
                )
                visual_vector = None
            ...
            try:
                ...
                log.warning(
                    "[Tracking] OpenCV not found, defaulting to 30.0 FPS"
                )
            except Exception as e:
                log.warning(
                    f"[Tracking] Failed to get FPS: {e}, defaulting to 30.0"
                )
