...
export const Voices: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      const [segData, clusterData] = await Promise.all([
  ...
  const handleTriggerClustering = async () => {
    ...
    try {
      await triggerVoiceClustering();
      await loadData();
    } catch (err) {
      console.error(err);
      setError('Failed to cluster voices');
    } finally {
  ...
  const handleMoveToCluster = async (clusterId: number) => {
    ...
    try {
      await moveVoiceToCluster(moveSegmentId, clusterId);
      await loadData();
      setMoveDialogOpen(false);
      setMoveSegmentId(null);
    } catch (err) {
      console.error(err);
      setError('Failed to move segment');
    }
  };
  ...
  const handleCreateNewCluster = async () => {
    if (!moveSegmentId) return;
    try {
      await createNewVoiceCluster([moveSegmentId]);
      await loadData();
      setMoveDialogOpen(false);
      setMoveSegmentId(null);
    } catch (err) {
      console.error(err);
      setError('Failed to create new cluster');
    }
  };
  ...
  const openMoveDialog = (segmentId: string) => {
    setMoveSegmentId(segmentId);
  ...
  const SegmentRow = ({ seg }: { seg: VoiceSegment }) => (
    <TableRow>
      ...
      <TableCell>
        {seg.speaker_name ? (
          <Chip label={seg.speaker_name} color="success" size="small" />
        ) : (
      ...
      <TableCell>
        <Tooltip title="Rename">
          <IconButton
            onClick={() => openRenameDialog(seg.id, seg.speaker_name)}
            size="small"
            color="primary"
          >
            <Edit fontSize="small" />
        ...
        <Tooltip title="Move">
          <IconButton onClick={() => openMoveDialog(seg.id)} size="small">
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
                ...
                >
                  <Box>
                    ...
                    <Typography
                      ...
                      display="block"
                    >
                      {cluster.segment_count} segment
                      {cluster.segment_count !== 1 ? 's' : ''}
                    </Typography>
                  </Box>
                  <Button
                    size="small"
                  ...
                  <Button
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      setMergeSourceId(cluster.cluster_id);
                    }}
                  >
                    Merge
                  </Button>
      ...
      >
        ...
        <DialogContent>
          <List>
            <ListItemButton onClick={handleCreateNewCluster}>
              <ListItemText
                primary="+ Create New Cluster"
                secondary="Create new speaker identity"
              />
            </ListItemButton>
            <Divider />
            {clusters.map(c => (
                ...
                .map(c => (
                  <ListItemButton
                    ...
                    sx={{
                      ...
                      borderColor: c.speaker_name
                        ? 'primary.main'
                        : 'transparent',
                      borderRadius: 1,
                      mb: 0.5,
                      bgcolor: c.speaker_name
                        ? 'action.selected'
                        : 'transparent',
                    }}
                  >
