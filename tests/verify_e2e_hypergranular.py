...
async def test_open_vocab_clothing():
    ...
    logger.success("Open-vocabulary clothing accepts ANY term (no hardcoded lists)")
...
async def test_open_vocab_audio():
    ...
    novel_sounds = ["duck quacking", "didgeridoo", "chainsaw", "alien spaceship"]
    ...
    logger.success("Open-vocabulary audio accepts ANY sound (no SOUND_CLASSES list)")
...
async def test_overlays_api_structure():
    ...
    overlays_file = Path(__file__).parent.parent / "api" / "routes" / "overlays.py"
...
async def test_frontend_structure():
    ...
    client_file = Path(__file__).parent.parent / "web" / "src" / "api" / "client.ts"
    ...
    search_file = Path(__file__).parent.parent / "web" / "src" / "pages" / "Search.tsx"
    ...
    video_player = Path(__file__).parent.parent / "web" / "src" / "components" / "media" / "VideoPlayer.tsx"
...
async def test_no_hardcoded_lists():
    ...
    files_to_check = {
        "clothing_attributes.py": ["CLOTHING_COLORS", "CLOTHING_ITEMS", "CLOTHING_PATTERNS"],
        ...
        if filepath.exists():
            ...
            for pattern in patterns:
                if re.search(rf'{pattern}\s*[:=]\s*\[', content):
                    raise AssertionError(f"HARDCODED LIST {pattern} FOUND IN {filename}!")
...
async def main():
    ...
    audit_results = {
        "Dead code wired (OCR, CLAP)": "YES" if "searcher_init" not in failed else "PARTIAL",
        "Hardcoded lists removed": "YES" if "hardcoded_lists" not in failed else "NO",
        "New Physics modules active": "YES" if "searcher_init" not in failed else "NO",
        "Frontend toggles implemented": "YES" if "frontend_structure" not in failed else "NO",
        "Open-vocab clothing": "YES" if "open_vocab_clothing" not in failed else "NO",
        ...
        "Dependency fix applied": "YES" if "dependency_fix" not in failed else "NO",
    }

    for check, result in audit_results.items():
        status = "✅" if result == "YES" else ("⚠️" if result == "PARTIAL" else "❌")
