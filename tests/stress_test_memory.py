...
def create_test_frames(n: int = 8, size: tuple = (224, 224)) -> list[np.ndarray]:
    """Create synthetic RGB frames for testing."""
    return [np.random.randint(0, 255, (*size, 3), dtype=np.uint8) for _ in range(n)]
...
async def test_temporal_analyzer() -> dict:
    ...
    return {"module": "TimeSformer", "before": before, "peak": peak, "after": after}
...
async def test_depth_estimator() -> dict:
    ...
    if result.get("stats"):
        print(f"   Depth range: {result['stats']['min_depth']:.2f} - {result['stats']['max_depth']:.2f}")
    
    estimator.cleanup()
    cleanup_vram()
    after = get_vram_info()[0]
    
    return {"module": "DepthAnything", "before": before, "peak": peak, "after": after}
...
async def test_clothing_detector() -> dict:
    ...
    frame = create_test_frames(1)[0]
    result = await detector.detect_attributes(frame)
    peak = get_vram_info()[0]
    
    if result:
        print(f"   Detected: {result.get('upper_body', {}).get('item', 'unknown')}")
    
    detector.cleanup()
    cleanup_vram()
    after = get_vram_info()[0]
    
    return {"module": "CLIP/Clothing", "before": before, "peak": peak, "after": after}
...
async def test_audio_events() -> dict:
    ...
    result = await detector.detect_events(audio, sr=16000)
    peak = get_vram_info()[0]
    ...
    for cycle in range(1, cycles + 1):
        ...
        for test_fn in test_funcs:
            try:
                ...
                print(f"   ✅ {result['module']}: Peak {result['peak']:.2f}GB → After {result['after']:.2f}GB")
            except Exception as e:
                print(f"   ❌ {test_fn.__name__} failed: {e}")
                cycle_results.append({"module": test_fn.__name__, "error": str(e)})
