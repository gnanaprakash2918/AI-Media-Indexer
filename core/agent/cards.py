...
VISION_AGENT_TOOLS = [
    AgentToolCard(
        ...
        parameters=[
            ...
            ToolParameter("temporal_context", "string", "Previous frame descriptions", required=False),
        ]
    ),
    AgentToolCard(
        name="track_concept",
        ...
        parameters=[
            ...
            ToolParameter("concept", "string", "Concept to track (e.g., 'red car', 'person')"),
        ]
    ),
    AgentToolCard(
        name="detect_faces",
...
AUDIO_AGENT_TOOLS = [
    AgentToolCard(
        ...
        parameters=[
            ...
            ToolParameter("language", "string", "Language code (e.g., 'ta', 'hi', 'en')", required=False),
        ]
    ),
    AgentToolCard(
...
SEARCH_AGENT_TOOLS = [
    AgentToolCard(
        ...
        parameters=[
            ...
            ToolParameter("person", "string", "Person name to filter by (HITL-assigned)", required=False),
...
def get_vision_agent_card(base_url: str = "http://localhost:8000") -> "AgentCard":
    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(id="analyze_frame", name="Analyze Frame", description="Dense frame analysis", tags=["vision"]),
            AgentSkill(id="track_concept", name="Track Concept", description="SAM3 tracking", tags=["segmentation"]),
            AgentSkill(id="detect_faces", name="Detect Faces", description="InsightFace 512D", tags=["identity"]),
        ],
    )
...
def get_audio_agent_card(base_url: str = "http://localhost:8000") -> "AgentCard":
    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(id="transcribe", name="Transcribe", description="ASR with Indic support", tags=["audio"]),
            AgentSkill(id="diarize", name="Diarize Speakers", description="Speaker identification", tags=["audio"]),
        ],
    )
...
def get_search_agent_card(base_url: str = "http://localhost:8000") -> "AgentCard":
    ...
    return AgentCard(
        ...
        default_output_modes=["application/json"],
        skills=[
            AgentSkill(id="search_scenes", name="Search Scenes", description="Complex visual search", tags=["search"]),
            AgentSkill(id="search_dialogue", name="Search Dialogue", description="Transcript search", tags=["search"]),
        ],
    )
