        ...
        try:
            ...
            while proc.returncode is None:
                ...
                if mem_mb > self.max_memory_mb:
                    log(f"[Sandbox] Memory limit exceeded! {mem_mb:.1f}MB > {self.max_memory_mb}MB. Killing PID {proc.pid}.")
                    proc.kill()
                    raise ResourceExceededError(f"Memory limit exceeded: {mem_mb:.1f}MB")
                
                # 2. Timeout check
                if time.time() - start_time > self.timeout_seconds:
                    log(f"[Sandbox] Timeout exceeded! {self.timeout_seconds}s. Killing PID {proc.pid}.")
                    proc.kill()
                    raise ResourceExceededError(f"Timeout exceeded: {self.timeout_seconds}s")
