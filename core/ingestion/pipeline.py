...
from core.processing.vision.vision.extractor import FrameExtractor
from core.processing.vision.vision.frame_sampling import TextGatedOCR
from core.processing.vision.vision.identity import FaceManager, FaceTrackBuilder
from core.processing.metadata.metadata import MetadataEngine

from core.processing.metadata.metadata.prober import MediaProbeError, MediaProber
from core.processing.vision.vision.scene_detector import (
    detect_scenes,
    extract_scene_frame,
    TransNetV2,
)
from core.processing.vision.vision.segmentation import Sam3Tracker
from core.processing.analysis.temporal_context import (
    SceneletBuilder,
    TemporalContextManager,
)
from core.processing.analysis.text_utils import parse_srt
from core.processing.audio.transcriber import AudioTranscriber

from core.processing.vision.vision import VisionAnalyzer
from core.processing.audio.voice import VoiceProcessor
from core.schemas import MediaType
from core.storage.db import VectorDB
from core.storage.identity_graph import identity_graph
...
class IngestionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.ocr_engine = OCRProcessor(lang="multilingual", use_gpu=True)
        self.text_gate = TextGatedOCR()
        self.extractor = FrameExtractor()
        self.db = VectorDB(
            backend=qdrant_backend,
    ...
    def _cleanup_memory(self, context: str = "") -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to log VRAM status: {e}")
    ...
    async def _process_audio(self, path: Path) -> None:
        ...
        if not audio_segments:
            ...
            use_lyrics_mode = False
            try:
                from core.processing.audio.content_classifier import (
                    get_content_classifier,
                )
            ...
            if settings.use_indic_asr and detected_lang in indic_languages:
                ...
                indic_transcriber = None
                try:
                    from core.processing.audio.indic_transcriber import (
                        IndicASRPipeline,
                    )
    ...
    async def _process_voice(self, path: Path) -> None:
        ...
        try:
            ...
            # Group segments by local speaker label first
            from collections import defaultdict
            from core.processing.audio.voice import compute_speaker_centroid
            ...
            for _idx, seg in enumerate(voice_segments or []):
                ...
                try:
                    if not hasattr(self, "_ser_analyzer"):
                        from core.processing.audio.speech_emotion import (
                            SpeechEmotionAnalyzer,
                        )
    ...
    async def _process_frames(
        ...
        # Use the configured LLM provider from settings
        from core.processing.vision.extractor import FrameExtractor
        from llm.factory import LLMFactory
    ...
    async def _process_scene_captions(
        ...
        try:
            ...
            if self._cached_scenes and self._cached_scenes_path == str(path):
                ...
                raw_scenes = []
                from core.processing.vision.scene_detector import SceneInfo
        ...
        for idx, scene in enumerate(scenes):
            ...
            if frame_bytes:
                try:
                    ...
                    from core.processing.analysis.deep_research import (
                        get_deep_research_processor,
                    )
                ...
                if frame_img is not None:
                    ...
                    try:
                        ...
                        if getattr(
                            ...
                            if not hasattr(self, "_moderator"):
                                from core.processing.vision.content_moderation import (
                                    VisualContentModerator,
                                )
                        ...
                        if getattr(settings, "enable_time_extraction", False):
                            if not hasattr(self, "_clock"):
                                from core.processing.vision.clock_reader import (
                                    ClockReader,
                                )
    ...
    async def _post_process_video(self, path: Path, job_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                from core.processing.vision.scene_detector import detect_scenes
