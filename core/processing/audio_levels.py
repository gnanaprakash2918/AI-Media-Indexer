...
class AudioLoudnessAnalyzer:
    ...
    async def _lazy_load(self) -> bool:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                log.info(
                    f"[Loudness] Meter initialized (SR={self.sample_rate})"
                )
                return True
    ...
    def _compute_dynamic_range(
        ...
        return float(
            np.percentile(rms_values, 95) - np.percentile(rms_values, 10)
        )
    ...
    async def detect_loud_moments(
        ...
        log.info(
            f"[Loudness] Found {len(loud_moments)} moments above {threshold_spl} dB SPL"
        )
        return loud_moments
