...
class TransNetV2:
    ...
    def _lazy_load(self) -> bool:
        ...
        try:
            ...
            if local_path.exists():
                 ...
                 try:
                     log.info("[TransNetV2] Downloading model from 'elya5/transnetv2'...")
                     ...
                     log.warning(f"[TransNetV2] Download failed: {e}. Please manually download 'transnetv2.onnx' to 'models/'.")
                     return False
            ...
            log.warning(f"[TransNetV2] Initialization failed (falling back to scenedetect): {e}")
            return False
    ...
    def predict_video(
        ...
        if not self._lazy_load():
            log.warning("[TransNetV2] Model not loaded, falling back to scenedetect")
            return []
        ...
        while True:
            ...
            if len(frames_buffer) >= 100:
                batch = np.array(frames_buffer, dtype=np.float32)[np.newaxis, ...]
                batch = batch.transpose((0, 2, 3, 4, 1)) # Possibly N, H, W, C ?? Check model spec
