...
class MusicAnalysis:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "beat_times": [
                round(t, 3) for t in self.beat_times[:100]
            ],  # Limit for storage
        }
...
class MusicStructureAnalyzer:
    ...
    def analyze_file(self, audio_path: Path) -> MusicAnalysis:
        ...
        try:
            ...
            log.info(
                f"[MusicStructure] Found {len(sections)} sections at {analysis.global_tempo:.1f} BPM"
            )
            return analysis
    ...
    def analyze_array(
        ...
        try:
            ...
            sections = self._detect_sections(
                audio, sr, tempo, beat_times, duration
            )
    ...
    def _detect_sections(
        ...
        try:
            ...
            bound_times = librosa.frames_to_time(
                bounds, sr=sr, hop_length=self.hop_length
            )
        except Exception:
        ...
        for i in range(len(bound_times) - 1):
            ...
            start_frame = librosa.time_to_frames(
                start, sr=sr, hop_length=self.hop_length
            )
            end_frame = librosa.time_to_frames(
                end, sr=sr, hop_length=self.hop_length
            )
            ...
            segment_contrast = float(
                np.mean(spectral_contrast[:, start_frame:end_frame])
            )
            ...
            sections.append(
                MusicSection(
                    ...
                    energy=segment_energy
                    / max(avg_energy, 0.001),  # Normalized energy
                )
            )
    ...
    def _detect_vocals(self, y: np.ndarray, sr: int) -> bool:
        ...
        try:
            ...
            harmonic_ratio = float(np.sum(np.abs(harmonic))) / (
                float(np.sum(np.abs(y))) + 1e-6
            )
