...
class ContentClassifier:
    ...
    def _check_ina_available(self) -> bool:
        ...
        try:
            ...
            log("[ContentClassifier] inaSpeechSegmenter not installed, using fallback")
            return False
    ...
    def _classify_with_ina(self, audio_path: Path) -> list[AudioRegion]:
        ...
        try:
            ...
            # Log summary
            speech_dur = sum(r.duration for r in regions if r.content_type == ContentType.SPEECH)
            music_dur = sum(r.duration for r in regions if r.content_type == ContentType.MUSIC)
            silence_dur = sum(r.duration for r in regions if r.content_type == ContentType.SILENCE)
            total = speech_dur + music_dur + silence_dur
            
            if total > 0:
                log(f"[ContentClassifier] {len(regions)} regions: "
    ...
    def _classify_fallback(self, audio_path: Path) -> list[AudioRegion]:
        ...
        try:
            ...
            if duration > 0:
                log(f"[ContentClassifier] Fallback mode: treating {duration:.1f}s as potential speech")
                return [AudioRegion(
                    content_type=ContentType.SPEECH,
                    start=0.0,
                    end=duration,
    ...
    def get_speech_regions(self, regions: list[AudioRegion]) -> list[AudioRegion]:
        """Filter to only speech regions."""
        return [r for r in regions if r.content_type == ContentType.SPEECH]
    
    def get_music_regions(self, regions: list[AudioRegion]) -> list[AudioRegion]:
