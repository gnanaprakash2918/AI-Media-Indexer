...
async def warmup_models():
    ...
    try:
        ...
        transnet = TransNetV2()
        tasks.append(asyncio.create_task(_warmup_component(transnet, "TransNet V2")))
...
def _print_status_report():
    ...
    try:
        # Check TransNet
        t_status = "OK" if Path("models/transnetv2.onnx").exists() else "MISSING"
        ...
        print("="*50 + "\n")
        print("Note: 'Lazy' models load on first use. Failures will be logged but won't crash the server.\n")
